...
def sol_stake(agent, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Staking failed: {str(e)}")
        return False
...
def sol_lend(agent, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Lending failed: {str(e)}")
        return False
...
def request_faucet_funds(agent, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Faucet request failed: {str(e)}")
        return False
...
def sol_get_tps(agent, **kwargs):
    ...
    try:
        ...
        logger.info(f"Current TPS: {result}")
        return result
    except Exception as e:
        logger.error(f"❌ TPS fetch failed: {str(e)}")
        return None
...
def get_token_data_by_ticker(agent, **kwargs):
    ...
    try:
        ...
        logger.error(f"❌ Token data fetch failed: {str(e)}")
        return None
