...
class EvmContractHelper:
    @staticmethod
    async def read_contract(web3: Web3, contract_address: str, method: str, *args) -> Any:
    ...
    @staticmethod
    async def _call(web3: Web3, private_key: str, contract_address: str, method: str, value: int=0, *args) -> Any:
        logger.info(f"STUB: Call contract {contract_address} method {method} with {args}")
        abi = await EtherscanHelper.get_contract_abi(contract_address)
        contract = web3.eth.contract(address=contract_address, abi=abi)
